...
function PreAcep(Id_sol,estatus,action){

if(action == "Rechazado"){
  $.ajax({url: "GetResults.php?Id_solicitud="+Id_sol+"&action="+action+"&function=PreAcep", success: function(result){            
        //header("Refresh:0");
        location.reload();
        alert("Solicitante Rechazado");        
      }});
}else if(action == "Pre-aceptado"){
  if(estatus == 'Pre-aceptado'){
      alert("Solicitante fue Pre-aceptado anterior mente");
    }else{    
  
      $.ajax({url: "GetResults.php?Id_solicitud="+Id_sol+"&action="+action+"&function=PreAcep", success: function(result){            
        //header("Refresh:0");
        location.reload();
        alert("Solicitante Pre-Aceptado");        
      }});
...
$SolOffers = GetSolOffer($Oferta_id);
...
?>
    ...
    <section class="ftco-section">
		<div class="container">
			...
			<div class="row">
				<div class="col-md-12">
					<h4 class="text-center mb-4">Solicitantes</h4>
                  ...
                  <th>Acciones</th>
					    ...
					    <tbody>
<?php
   while ($Sol = $SolOffers->fetch_assoc()) {
    
?>             
					      <tr>
					        <th scope="row" class="scope" ><?php echo $Sol['USRS_NAME']." ".$Sol['USRS_LASTNAME']?></th>
					        <td><?php echo $Sol['USRS_EMAIL']?></td>
                  <td><?php echo $Sol['telefono']?></td>
					        <td><?php echo $Sol['Profecion']?></td>
                  <td><?php echo $Sol['Estatus']?></td>
					        <td><a target="_black" href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] . '/meraki/Registro/' . $Sol['cv_url']; ?>" class="btn btn-primary">CV</a></td>
                  <td>
<?php
if($Sol['Estatus'] =="Pendiente"){
?>
                      <button id="PreAc" onclick="PreAcep('<?php echo $Sol['id_solicitudes']; ?>','<?php echo $Sol['Estatus']; ?>','Pre-aceptado')" class="btn btn-primary">Pre-aceptar</button>
<?php
}else if($Sol['Estatus']== "Pre-aceptado"){
?>
                      <button id="PreAc" onclick="PreAcep('<?php echo $Sol['id_solicitudes']; ?>','<?php echo $Sol['Estatus']; ?>','Aceptado')" class="btn btn-primary">Aceptar</button>
<?php  
}
?>
<?php
...
?>
                      <button onclick="PreAcep('<?php echo $Sol['id_solicitudes']; ?>','<?php echo $Sol['Estatus']; ?>','Rechazado')" class="btn btn-primary">Rechazar</button>
<?php  
}
?>
                  </td>
